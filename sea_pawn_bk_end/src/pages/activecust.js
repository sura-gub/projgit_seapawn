...
function ActiveCust() {
    ...
    useEffect(() => {
        ...
        const fetchLoan = async () => {
            try {
              const response = await fetch('http://localhost:5000/getLoans');
              if (response.ok) {
  ...
  for (let i = 1; i <= Math.ceil(loan.length / itemsPerPage); i++) {
    ...
    return (
        <div className='bghome'>
            ...
            <div style={{ zoom: 0.8, margin: '0px 10px' }} className='vh-100'>
                        ...
                        {currentItems.map((Loan, index) => {
                            ...
                            return (
                                <tr key={index} className='text-start'>
                                    ...
                                    <td style={{ backgroundColor: '#fff0', fontWeight: '400', whiteSpace: 'wrap' }}>{Loan.aprox_value}</td>
